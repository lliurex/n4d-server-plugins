...
class LdapUser:
	...
	def __init__(self,properties):
		...
		if not self.properties.has_key("description"):
...
class LdapManager:
	...
	def __init__(self,llxvar):
		
		print("[LDAPMANAGER] INIT")
		self.llxvar=llxvar
	...
	def reset_connection_variable(self):
		...
		if url!=None:
			...
			print("[LDAPMANAGER] Starting connection thread...")
	...
	def _rcv_thread(self):
		...
		while True:
			...
			time.sleep(90)
			print("[LDAPMANAGER] Restoring connection variable...")
			self.restore_connection=True
	...
	def get_xid_counters(self,uid="*"):
		...
		for item in result:
			...
			if "x-lliurex-ou-properties" in  properties_dic['objectClass'] and properties_dic.has_key("x-lliurex-xid-counter"):
				self.xid_counters[properties_dic['ou'][0]]=properties_dic['x-lliurex-xid-counter'][0]
	...
	def get_samba_id_t(self):
		...
		for count in range(0,10):
			...
			try:
				...
				print("[LDAPMANAGER] Samba ID ready.")
				return True
			except Exception as e:
				print("[LDAPMANAGER] Get Samba ID failed: %s.")
				if count < 9:
					print("[LDAPMANAGER] Retrying in 2 sec...")
					time.sleep(2)
	...
	def connect(self):
		...
		if self.restore_connection:
			...
			sys.stdout.write("[LDAPMANAGER] Connecting to ldap ... ")
			url=None
			remote=False
			try:
				url=self.llxvar("MASTER_SERVER_IP")
	...
	def add_user(self,generic_mode,plantille,properties):
		...
		if (plantille=="Students"):
			...
			if not properties.has_key("userPassword"):
				password="".join(random.sample(string.letters+string.digits, 4))
		...
		if user.properties.has_key("userPassword"):
		...
		if "sambaNTPassword" not in user.properties:
			user.properties['sambaNTPassword']=smbpasswd.nthash(password)
		if "sambaLMPassword" not in user.properties:
			user.properties['sambaLMPassword']=smbpasswd.lmhash(password)
		...
		if not user.properties.has_key("uidNumber"):
	...
	def change_password(self,path,password):
		...
		try:
			...
			self.modify_value(path,"sambaLMPassword",smbpasswd.lmhash(password))
	...
	def add_generic_users(self,plantille,group_type,number,generic_name,pwd_generation_type,pwd=None):
		...
		for i in range(number):
			...
			if(pwd_generation_type==LdapManager.RANDOM_PASSWORDS):
				...
				generated_pwd="".join(random.sample(string.letters+string.digits, 4))
	...
	def get_students_passwords(self):
		...
		for item in result:
			...
			try:
				path,dic=item
				user={}
				if dic.has_key("userPassword"):
					user['passwd']=dic["userPassword"][0]
				if dic.has_key("sambaNTPassword"):
					user['sambaNTPassword']=dic["sambaNTPassword"][0]
				if dic.has_key("sambaLMPassword"):
