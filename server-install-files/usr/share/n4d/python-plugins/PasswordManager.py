...
class PasswordManager:
	...
	def write_file(self,f=None):
		...
		for user in self.users:
			if type(self.users[user]["cn"])!=unicode:
				self.users[user]["cn"]=self.users[user]["cn"].decode("utf-8")
			if type(self.users[user]["sn"])!=unicode:
				self.users[user]["sn"]=self.users[user]["sn"].decode("utf-8")
		
		data=unicode(json.dumps(self.users,indent=4,encoding="utf-8",ensure_ascii=False)).encode("utf-8")
